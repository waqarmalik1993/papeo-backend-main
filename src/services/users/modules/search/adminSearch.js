...
exports.adminSearch = async (user, query) => {
  let skip = 0;
  let limit = 10;
  ...
  if (query.include) {
    ...
    if (query.include.includes("locked")) {
      filterAndConditions.push({ locked: { $eq: true } });
    }
    if (query.include.includes("muted")) {
      filterAndConditions.push({
        $or: [
          { "restrictions.reportMedia": true },
          { "restrictions.createParties": true },
          { "restrictions.uploadMedia": true },
          { "restrictions.commentMedia": true },
          { "restrictions.participateInParties": true },
          { "restrictions.login": true },
        ],
      });
    }
  }
